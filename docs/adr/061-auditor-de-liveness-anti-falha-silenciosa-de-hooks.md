...
## EMENDA 2026-06-10 (plano anti-bypass cross-IA, item E1) — forma executável única do auditor

**Motivação (análise dos erros sistêmicos do Gemini + pedido do dono "garantias existem → garantir que sejam sempre executadas, não bypassadas"):** o auditor **declarava** os gates inertes mas o remédio era **N passos manuais espalhados** (sync, agnosticismo, boot-scan, versão) — cada um skippável. Falha real 2026-06-08: boot-scan **assumido vazio** sem executar. Régua §0: **funde N→1**, não adiciona superfície.

**Mecanismo:** `tools/boot_check.py` — comando **único, em Python (imune ao veto de EDR)**, que o agente roda no `/start-session`. Subprocess-reusa os checks existentes (`check_repo_sync.py`, `check_core_agnostic.py`, `cross_ai_hub.py boot-scan`) + âncora de versão canônica (anti "versão fantasma" — erro real do boot Gemini que declarou `v3.1` inexistente). Grava **prova checável** em `.claude/boot-proof.json` (gitignored — por sessão/máquina) e, com `--session <id>`, **carimba os liveness keys** dos gates que executou + a key `boot-check` → o banner do route-gate **nag-a até o agente rodar** e então se cala. Canário fail-closed: `tools/test_boot_check.py`. **Enforcement honesto = `manual`** (agente roda; onde EDR veta hooks, não há fail-closed possível) — débito de mecanização **declarado**, não escondido; a alavanca anti-bypass é o nag de liveness (não-vetoável) + doutrina no start-session. Não suplanta a decisão original (auditor permanece); **emenda a §Implementação** com a forma consolidada.
