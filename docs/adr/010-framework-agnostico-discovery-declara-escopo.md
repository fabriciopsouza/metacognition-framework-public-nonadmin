...
- **Relaciona-se a:** ADR-007 (régua §0), ADR-009 (método sênior + auto-observação), ADR-005 (modos de execução / HITL via execution-modes)
...
- `PROMPT-CHAT-WEB-v4.2.md` (raiz, parte do distribuído) trazia identidade hardcoded ("conteúdo de domínio de um cliente") em §1 e §5.

**Diagnóstico operacional (2026-05-28):** as falhas observadas no um caso real (aplicar uma norma de um segmento como se regulasse outro; usar uma norma de um setor como âncora indevida em outro) **não foram falhas do "método cross-domain reconciliation"**. Foram sintomas de **vazamento cross-projeto** — o agente carregava nas próprias regras do framework convenções de outros projetos do mesmo dono (dois clientes de setores distintos do mesmo dono), e ativava-as por similaridade semântica em qualquer projeto. O framework declarava-se agnóstico, mas operava com viés.
...
| 2 | Manter exemplos hardcoded como "pedagógicos" rotulados como `ex.:` | Mínimo esforço; exemplos ajudam quem nunca viu. | Não resolve o problema real — exemplos viram âncoras cognitivas (memória do agente os promove a regra). O um caso real demonstrou isto: ALCOA+ entrou como âncora apesar de rotulado como exemplo. |
...
| `PROMPT-CHAT-WEB-v4.2.md` | §1 Identidade e §5 Domínio: substituir conteúdo hardcoded (um cliente regulado/SUA-ORG/uma ferramenta de BI/ANVISA/GAMP) por **template com placeholders** instruindo o usuário a customizar. Manter estrutura. | Subtração massiva + template |
...
`~/.claude/projects/<seu-projeto>/memory/senior-discovery-method.md`:
...
Diagnóstico (um caso real 2026-05-27): 3 violações file-first foram apontadas pelo dono, NÃO auto-observadas pelo agente. Chamar isso de "auto-observação" overstates a capability. O princípio 11 (ADR-009) foi reescrito em v1.11.0 para refletir realidade:
...
- Memória-fonte: `[[senior-discovery-method]]` (purgada nesta v1.11.0), `[[framework-gaps-from-case]]` (audit trail das 3 violações cross-projeto).
...
- **Princípio 11 honesto NÃO endereça file-first** (ADV-3 qa-critic round 1): o rename de "auto-observação" → "observação meta-cognitiva" corrige a representação de capacidade, mas a causa-raiz das 3 violações no um caso real foi violação da regra inviolável #3 (file-first), não auto-observação fracassada. As duas correções são independentes; enforcement de file-first segue regra inviolável existente e fica fora desta v1.11.0.
