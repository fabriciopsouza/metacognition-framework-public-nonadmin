...
- **Substitui:** nenhum · **Substituído por:** nenhum (integral) · **EMENDADO por ADR-051** (2026-06-01): o **passo-1 (filtro de entrada)** — que mandava "não inferir por sinais semânticos" e exigia fonte declarada — era o **bug provado** (o método nunca carregou no caso o caso real/um cliente regulado). ADR-051 muda o gatilho para **disparar também por sinais de stake INFERIDOS** (`context-signals.txt`) e **mecaniza** vigência+pertinência no `context-brief` (gate `check_context_brief.py`).
...
3. **`anti-hallucination/SKILL.md` ganha 1 anti-pattern**: "citar norma/spec/doc externa sem checar vigência (em vigor? alterada? revogada?)" — atende item B dos insights o caso real; integrado ao núcleo SSoT, não duplicado.
...
3. **Validity-check externa pode exigir web research**: explorer é repo-only; gap conhecido (item J de `[[framework-gaps-from-case]]`). Tratado em follow-up; nesta v, validity-check é princípio + delegação manual ao WebSearch quando necessário.
...
- **Validação:** qa-critic adversarial em rounds até LIMPO + reaplicação do método em próximo case real (o fechamento do caso real ou outro domínio) verificando se as ~10 correções do dono caem para ≤3.
...
- **o fechamento do caso real** (validar a esteira completa): implementar REQ-001..007 + qa-critic + run + os critérios de aceite. Fora deste repo.

## Referências

- Memórias-fonte: `[[senior-discovery-method]]` (substância), `[[framework-self-improvement]]` (auto-observação), `[[framework-gaps-from-case]]` (audit trail).
- Case real: `repo privado do mantenedor` branch `branch do caso real` (commits `<commit>`, `<commit>`).
