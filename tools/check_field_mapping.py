...
CONFIRM_RX = re.compile(
    ...
    r"registrad[oa]\s+pelo\s+dono|dono\s+confirmou|"
    # "confirmação: <qualquer coisa>" aceitava "confirmação: pendente" como confirmado — PASS falso
    # achado ao usar o gate num plano real (25/09/2026, ADR-118). Estado de espera não é confirmação.
    r"confirma[cç][aã]o\s*:\s*(?!\(?\s*(?:pendente|a\s+confirmar|aguardando|n[aã]o\b|n/?a\b|nenhuma|tbd|todo|"
    r"\?|<|-|—))\S",
    re.IGNORECASE)
...
def check_spec(path):
    try:
        text = spec_fonte.ler_parte(path, "requisitos")  # formato antigo ou arquivo único (ADR-117)
    except OSError as e:
        return False, True, [(f"<io {e}>", ["io"])]
    if text is None:
        return False, True, [("<parte 'Requisitos' ausente no arquivo único>", ["parte"])]
...
def main(argv):
    ...
    if not targets:
        targets = [t for t in glob.glob(os.path.join(ROOT, "docs", "specs", "**", "requirements.md"),
                                        recursive=True) + spec_fonte.alvos_unicos(ROOT)
                   if "_template" not in t and not spec_fonte.sombreado_por_unico(t)]
    if not targets:
